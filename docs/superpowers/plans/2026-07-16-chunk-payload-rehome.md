...
- **The one hardware-verified-only risk is cache-line ALIGNING the payload (Stage 2 Task 3, optional).** No CI exercises the DMA/cache-maintenance path (sim has no cache instructions; `padsweep` covers layout-invariance, not DMA). *Preserving* ≥cache-line guards (Tasks 1–2) keeps the existing DMA behaviour by construction and is low-risk; *changing* the payload's alignment relative to cache lines (Task 3) alters the DMA rounding and is verifiable only on real cortex-a9 hardware. Task 3 is optional and recommended for deferral unless a measured benefit justifies the ear-check.
...
### Task 3 (OPTIONAL — HARDWARE-GATED — recommend defer): cache-line-align the payload

Aligning `payload_` to a cache-line boundary (`kPayloadOffset` a multiple of `CACHE_LINE_SIZE`) would let the DMA front-rounding stop over-reaching before the payload — a marginal tidiness gain. **It changes the payload's alignment relative to cache lines, which alters the DMA cache-maintenance rounding, and NO CI exercises that path — it is verifiable only on real cortex-a9 hardware.** The front guard is still needed for the application under-read regardless. Given marginal benefit vs. a device-only-crackle risk, **recommend NOT doing this** unless a measured problem motivates it.

- [ ] **Step 1 (only if pursued):** Set `kPayloadOffset` to a `CACHE_LINE_SIZE` multiple; keep both guards ≥ one cache line.
- [ ] **Step 2:** Build; goldens/padsweep (a signal, not a gate — the render is unaffected, but the DMA rounding is).
- [ ] **Step 3:** **On-device ear-check REQUIRED before merge:** load + stream a large non-native-format sample under memory pressure and confirm no crackle (the exact failure mode the guards prevent). This is the human's hardware gate.
- [ ] **Step 4:** Commit `refactor(audio-stream): cache-line-align the chunk payload` (only after the ear-check passes).
...
- Per-task: `./dbt build Debug` + `./dbt test` (20/20) + goldens + `padsweep`. Stage 1 + Stage 2 Tasks 1–2 target bit-exact.
- **The DMA/cache-maintenance path is not CI-covered.** Stage 2 preserves the guard geometry by construction (low risk); the optional Task 3 alignment is the only step that needs an on-device ear-check (streamed non-native sample under memory pressure). That is the user's hardware gate.
...
- Open risks to surface at review: (a) Stage 2 Task 2's `kPayloadOffset`/`kTrailingGuard` values must keep both guards ≥ `CACHE_LINE_SIZE` — the one place a subtle error would reintroduce the exact DMA hazard the guards prevent (and it is NOT golden-catchable). (b) The recorder's explicit `Cluster::size + 5` over-reads and the `-4 + byteDepth` front-underreads must keep landing in guard slack after re-home — the repoint (Stage 1) is address-preserving so this holds, but Stage 2's offset change must keep ≥ the same slack. (c) Confirm `dest` at every construct callback is the true slot base (the value the slab returned), not an interior pointer, so `payload_` has whole-slot provenance.
