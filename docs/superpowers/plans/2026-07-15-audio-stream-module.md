...
- **Phase 4 — introduce `SampleStream`.** Migrate the residency table + `getCluster` dispatch + stream handle off `Sample`/`SampleCluster`; `SampleHolder` and the RT reader lease through it; `makeReadSource` ownership moves here. **Required here (deferred from Phase 1):** `SampleStream` must *hold* the `ReadSource` on the per-`Sample` object rather than constructing one per `readClusterData` call. Phase 1's Task 4 left `makeReadSource` doing a `std::make_unique` per call, which introduced a small (~−92 dBFS, inaudible, converging) allocation-timing divergence on the `highsiderr` fixture (cordae/icoustic stayed bit-exact; padsweep invariant). Holding the source on `SampleStream` eliminates the per-call heap alloc structurally and should remove that divergence. Gate: golden — expect this to *improve* highsiderr back toward pre-Phase-1 behavior; ear-check if needed.
