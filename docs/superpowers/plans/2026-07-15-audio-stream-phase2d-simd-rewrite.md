...
- [ ] **Step 1: Wire argon+SIMDe into the convert spec driver.** Mirror how `tests/dsp/CMakeLists.txt` (the existing dual-arch SIMD spec dir) makes argon + SIMDe available to a CppSpec driver (FetchContent/link/include + the `sim/compat` `<arm_neon.h>` intercept). Confirm a trivial `#include "argon.hpp"` compiles in the spec build FIRST (add it to `convert.h` behind the real use in the next step).
