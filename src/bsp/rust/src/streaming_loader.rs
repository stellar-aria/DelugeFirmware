...
// Task 5). See `include/libdeluge/streaming_fill.h`'s doc for the C-side contract. `FillContext`,
// `FILL_CONTEXT_CAP`/`FILL_CONTEXTS`, `deluge_streaming_set_fill_context`, `fill_context_for`,
// `StreamingFillDescriptor`, and `DelugeChunkConvertState` all now live in the shared
// `deluge_sample_fill` crate (C2a Task 2) — imported below rather than defined in this file.
use deluge_sample_fill::{FillContext, StreamingFillDescriptor, fill_context_for};
...
mod prod {
    ...
    use core::ffi::c_void;
    use deluge_sample_fill::{DelugeChunkConvertState, StreamingFillDescriptor};
