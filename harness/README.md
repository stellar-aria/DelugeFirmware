...
| lens1-vt-sim | simulation | investigative | measures streaming-underrun margin against modeled SD latency; deterministic virtual time | nightly | ~25m |
| preemptive-race-tsan | sanitizer | gate | sustained playback-while-record must report no new data race under ThreadSanitizer | nightly | ~30m |
...
| golden-embassy-diff | differential | gate | the Embassy host BSP's mixdown must match the C-host golden renderer's for the same fixture | nightly | ~20m |
