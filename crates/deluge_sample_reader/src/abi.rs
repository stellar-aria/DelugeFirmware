...
//! (`include/libdeluge/sample_reader.h`). Task 1 (SR-U1) implements the lifecycle trio —
//! `open`/`seek`/`close` — by heap-boxing a [`crate::reader::Reader`]; `window`/`advance`/`ok` and
//! the stateless `deluge_sample_read` copy are later tasks (their signatures already exist in the
//! header — the header is the contract — just not yet backed here; calling them today would be a
//! link error, which is the correct failure mode for "not implemented yet").
//!
...
mod tests {
    ...
    /// module tests the raw C-ABI surface specifically, not `Reader`'s internal state.
    ...
    fn seek_through_the_abi_updates_current_frame_and_drops_a_held_lease() {
        ...
        // Reach through the opaque pointer to attach a real lease (mirroring what a future
        // `window()` would do), the same way `reader.rs`'s own tests do -- there is no public ABI
        // call yet that populates `held_lease` (that is `window()`, a later task).
    ...
    #[test]
    fn null_reader_is_tolerated_by_seek_and_close() {
        // SAFETY: `reader` is null (the case under test) -- both fns null-check before any deref.
        unsafe {
