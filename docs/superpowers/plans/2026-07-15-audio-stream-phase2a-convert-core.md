...
- [ ] **Step 3: Implement `convert_cluster_data`** in `convert.{h,cpp}`. Port the body of `Cluster::convertDataIfNecessary` (cluster.cpp:60-153) VERBATIM into the pure function, with exactly these substitutions:
  ...
  Keep the early-outs and all loop bounds identical. Do NOT include `AudioEngine`/`Sample`/`Cluster` headers in `convert.cpp` — the function stays dependency-light.

  **Naming — snake_case EVERY ported local (not just "new" ones).** The source (cluster.cpp:60-153) is legacy camelCase; the ported code must fully conform to the house convention, NOT preserve the source's casing. Rename every local as you port: `startPos`→`start_pos`, `startCluster`→`start_cluster`, `endPos`→`end_pos`, `endPosNow`→`end_pos_now`, `bytesBeforeStartOfCluster`→`bytes_before_start_of_cluster`, `bytesThatWillBeEatingIntoAnother3Byte`→`bytes_eating_into_another_3byte`, `bytesUnconvertedBeforeCluster`→`bytes_unconverted_before_cluster`, `endAtBytePos`→`end_at_byte_pos`, `endAtPosWithinCluster`→`end_at_pos_within_cluster`, `temp`→`temp` (already fine), `pos`→`pos` (fine), etc. When done, grep the new function for any lowercase-then-uppercase identifier and confirm zero camelCase locals remain.
