...
### Task 1: introduce `payload()` + the edge-slack contract, document the guards, enforce the invariants

**Files:** Modify `cluster.h` (both structs + the slack constants/accessors), `cluster.cpp` (static_asserts + any out-of-line accessor).

This task adds the typed payload view AND codifies the boundary-straddle over/under-read as a named, sized, `static_assert`-enforced **edge-slack contract** (folded in from the "Level 1" hardening discussion) — so the guards' two distinct jobs (DMA rounding vs. application edge-slack) are explicit and separately provable, and the `-4 + byteDepth` frame-cursor idiom gets a name.

- [ ] **Step 1 — `payload()`:** Add to `StreamedChunk` and `ComputedChunk`: `[[nodiscard]] std::span<std::byte> payload();` + a `const` overload, returning a span over the payload base (`reinterpret_cast<std::byte*>(data)`) of length `Cluster::size`. `///` Doxygen: it is the DMA'd cluster payload; its declared backing (`data`) is a placeholder over-allocated to `Cluster::size` in the slab slot.
- [ ] **Step 2 — the edge-slack contract:** Add named constants (in cluster.h) for the application's boundary-straddle reach: `kFrontSlackBytes = 4` (the `-4 + byteDepth` front-underread; ≤4 for byte_depth ≥ 1) and `kTrailingSlackBytes = 7` (the stitch `Cluster::size + 7` and recorder `+5` over-reads). Add the named cursor accessor `[[nodiscard]] std::byte* frame_read_origin(uint32_t pos, uint8_t byte_depth)` to each chunk (or one shared free helper over `payload()` if that reads cleaner) — returns `payload().data() + pos - 4 + byte_depth`, i.e. the pointer positioned so a 32-bit word read yields the `byte_depth`-byte sample frame left-justified. Doxygen: it may point up to `kFrontSlackBytes` into the leading guard for the first frame of a cluster (those bytes are don't-care), and is called at cluster-boundary cadence, not per sample. Add a `[[nodiscard]] std::span<std::byte> payload_with_trailing_slack()` (or a documented `payload().data() + Cluster::size + k` form + `kStitchTrailingBytes`) for the stitch/recorder trailing over-reads, so the trailing reach is expressed against a named span, not a bare offset.
- [ ] **Step 3 — document the guards:** Replace the terse `// MUST BE THE LAST TWO MEMBERS` with a doc block naming the guards' TWO distinct jobs — (a) DMA cache-line rounding absorption (≥`CACHE_LINE_SIZE`, since `data` is unaligned and the SD cache-maintenance rounds the range out to line boundaries) and (b) the application edge-slack (`kFrontSlackBytes`/`kTrailingSlackBytes`, the boundary-straddle frame reads) — and note the DMA guard dominates so the app slack rides for free.
- [ ] **Step 4 — enforce:** Add `static_assert`s in `cluster.cpp` for BOTH structs: `data` is the last member (`offsetof(T, data) + CACHE_LINE_SIZE == sizeof(T)`); `dummy` immediately precedes it (`offsetof(T, dummy) + CACHE_LINE_SIZE == offsetof(T, data)`); **the front guard covers both jobs** (`CACHE_LINE_SIZE >= kFrontSlackBytes` — i.e. `dummy`'s size ≥ the app front reach) and **the trailing guard covers both jobs** (`CACHE_LINE_SIZE >= kTrailingSlackBytes`); and `sizeof(T)` is unchanged (guard against silently perturbing the layout). Keep the existing `is_standard_layout`/`!is_polymorphic` asserts.
- [ ] **Step 5:** `./dbt build Debug` clean (the asserts must pass at current layout); `./dbt test`; goldens (cordae + icoustic) bit-exact; `padsweep`.
- [ ] **Step 6:** Commit `refactor(audio-stream): add payload() view + the edge-slack contract; enforce/document the chunk guards`.
...
**Transform:** `cluster->data` (pointer use) → `cluster->payload().data()`; `cluster->data[i]` → `cluster->payload()[i]`; `&cluster->data[i]` → `&cluster->payload()[i]` (or `cluster->payload().data() + i`); casts unchanged (`(int32_t*)&cluster->payload()[x]`). **The boundary-straddle sites route through the Task-1 named accessors, not raw arithmetic:** the ~5 `&cluster->data[pos] - 4 + byte_depth` front-underread sites (`sample.cpp:658,875`, `time_stretcher.cpp:744,1170`, `voice_sample.cpp:715`) → `cluster->frame_read_origin(pos, byte_depth)`; the stitch `Cluster::size + 7` self-span and the recorder `+Cluster::size`/`+5` over-reads → `payload_with_trailing_slack()` / the named trailing form. All addresses are byte-for-byte identical → bit-exact.
...
**Documented later step — "Level 2": codify the frame-read idiom itself.** This plan names the frame-cursor *setup* (`frame_read_origin`) and the slack contract, but the deeper convention — reading a 4-byte word and mask/shift/left-justifying it into a 2- or 3-byte fixed-point sample — is still scattered raw across the RT read path (`sample_low_level_reader`, `voice_sample`, `time_stretcher`, `sample_recorder`), including the per-sample hot loop. A typed `SampleFrame`/frame-cursor abstraction would make that explicit and testable, but it touches the hot loop (must stay zero-cost) and spans the DSP read path, so it belongs with the **synth/DSP extraction** work (see `[[synth-untangling-architecture]]`), not this payload re-home.
